    ...
    >
      {/* https://youtu.be/3hepJA56KF8?t=3227 */}
       {headerData?.map((item) => (
        ...
        key={item?.title}  
        href={item?.href}
        className={`hover:font-darkColor hoverEffect relative group ${pathname === item?.href && 'text-red-600' }`}
        >
          {item?.title}
          <span
          className={`absolute text-red-600 -bottom-0.5 left-1/2 w-0 h-0.5 bg-darkColor hoverEffect group-hover:w-1/2 group-hover:left-0 ${pathname === item?.href && 'w-1/2'}`}
        />
        <span
